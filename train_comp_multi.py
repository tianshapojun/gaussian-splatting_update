...
if not os.path.exists(logger_dir):
    os.mkdir(logger_dir) 
logger_path = os.path.join(logger_dir, "train_comp_0716_size_threshold2.log")
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + 0.1 * loss_dyn(image_dyn,mask_new) + 0.1 * loss_depth
        loss.backward()
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("./output/", result[0]+'_update_0716_size_threshold2')#unique_str[0:10])
