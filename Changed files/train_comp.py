...
import sys
from scene import Scene_comp, GaussianModel
...
if not os.path.exists(logger_dir):
    os.mkdir(logger_dir) 
logger_path = os.path.join(logger_dir, "train_comp_0703.log")
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene_comp(dataset, gaussians1,gaussians2)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gaussians2.o2w(viewpoint_cam.obj_rt)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                visibility_filter_1,visibility_filter_2 = visibility_filter[:gaussians1._xyz.shape[0]],visibility_filter[gaussians1._xyz.shape[0]:]
                radii_1,radii_2 = radii[:gaussians1._xyz.shape[0]],radii[gaussians1._xyz.shape[0]:]
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    #size_threshold2 = 10 if iteration > opt.opacity_reset_interval else None
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("./output/", result[0]+'_update_0703')#unique_str[0:10])
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene_comp, renderFunc, renderArgs):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    scene.gaussians2.o2w(viewpoint.obj_rt)
