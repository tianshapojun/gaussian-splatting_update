...
def render_set_interp(model_path, name, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration))
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #     rr +=1
        gaussians.o2w(view.obj_rt)
        rendering = render(view, gaussians, pipeline, background)["render"]
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        # gaussians.w2o(view.obj_rt)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams):
    with torch.no_grad():
        ...
        render_set_interp(dataset.model_path, "eval", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background)
