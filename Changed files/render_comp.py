...
import torch
from scene import Scene_comp
import os
from tqdm import tqdm
from os import makedirs
from gaussian_renderer.render_comp import render,render_double,render_obj
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    idxx = [i for i in range(1,10)]+[i for i in range(10,19)]+[i for i in range(20,29)] \
        +[i for i in range(30,39)]+[i for i in range(40,49)]+[i for i in range(50,59)]
    #views = [views[i] for i in idxx]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams):
    with torch.no_grad():
        ...
        scene = Scene_comp(dataset, gaussians1, gaussians2,load_iteration=iteration, shuffle=False)
        ...
        render_set_interp(dataset.model_path, "eval_statttttttttttttt", scene.loaded_iter, scene.getTrainCameras(), gaussians1, gaussians2,pipeline, background)
