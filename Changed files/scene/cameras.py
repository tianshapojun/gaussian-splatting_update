...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, depth, 
                 image_name, uid,
                 cx, cy, obj_rt, obj_quat,
                 trans=np.array([0, 0, 0.0]), scale=1.0, data_device = "cuda"
                 ):
        ...
        self.cx = cx 
        self.cy = cy
        self.obj_rt  = torch.tensor(obj_rt,device = self.data_device)
        self.obj_quat = torch.tensor(obj_quat,device = self.data_device)
        #self.obj_rt  = obj_rt
